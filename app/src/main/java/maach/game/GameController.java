...
public class GameController implements SensorEventListener, OnTouchListener
{
	...
	{
		levelData = new LevelData[N_LEVELS + 1];
		...
		// the 'extra' level
		levelData[10].normalButtonImage = GameController.renderer.loadImage(R.drawable.customlvl);
		levelData[10].pressedButtonImage = GameController.renderer.loadImage(R.drawable.customlvl_pressed);
		levelData[10].levelFile = R.raw.levelextra;
		levelData[10].levelBackground = 3;

		levelData[10].minFishForStars[1] = 0;
		levelData[10].minFishForStars[2] = 5;
		levelData[10].minFishForStars[3] = 10;
		levelData[10].minFishForStars[4] = 15;
		levelData[10].minFishForStars[5] = 20;
	...
	public static void loadNextGameLevel()
	{
		...
		else if (currentLevel == N_LEVELS)
		{
			// finished the extra level too!
			showMessage("Now more of your own levels!", Toast.LENGTH_LONG);
			showLevelsScreen();
		}
		else
		{
			showMessage("You have finished all regular levels! Now either try completing every level with 5 stars - or create your own custom levels :-)", Toast.LENGTH_LONG);
			showLevelsScreen();
		}
	}
...
class LevelsScreen extends MenuScreen
{
	...
	{	
		...
		// Ankan - for testing
		LevelButton customLevelButton = new LevelButton();
		customLevelButton.setPosition(520, 630);
		customLevelButton.setSize(250, 80);
		customLevelButton.level = GameController.levelData[10];
		customLevelButton.levelNumber = 10;
		customLevelButton.setImageHandles(customLevelButton.level.normalButtonImage, customLevelButton.level.pressedButtonImage);
		buttons.add(customLevelButton);
